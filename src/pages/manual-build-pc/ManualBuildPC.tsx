...
const ManualBuildPC = () => {
    ...
    const { selectedProducts: selectedProductsQuery } = router?.query || { selectedProducts: '{}' };
    const initialSelectedProducts = selectedProductsQuery ? JSON.parse(selectedProductsQuery as string) : {};

    const [selectedProducts, setSelectedProducts] = useState<{[key: string]: any}>(initialSelectedProducts);
    ...
    const [sortOption, setSortOption] = useState<'name' | 'price-asc' | 'price-desc'>('price-asc');
    ...
    const handleSelectClick = async (category: string, page: number = 1) => {
        ...
        const selectedParts = Object.entries(selectedProducts).map(([cat, product]) => ({
            name: product?.name || "",
            label: cat,
        }));
        try {
    ...
    const handleItemSelect = (product: any) => {
        setSelectedProducts((prev) => ({ ...prev, [currentCategory]: product }));
        setShowPopup(false);
    };
    ...
    useEffect(() => {
        ...
        setIsCompatible(true); // Placeholder
    }, [selectedProducts]);
        ...
        <div className="manual-build-pc-container grid grid-cols-8 gap-4 justify-center py-4 w-full bg-bgSecondary">
            <div className="p-4 bg-bgSecondary shadow-md border-t-2 border-slate-200 dark:bg-white dark:border-zinc-100 col-span-6">
                ...
                {itemData.map(({ label, imageSrc }) => {
                    ...
                    <ProductCard
                        key={label}
                        imageUrl={selectedItem?.imageUrl || "https://via.placeholder.com/150"}
                ...
                <div className="relative p-4 w-full max-w-2xl max-h-full">
                    ...
                    <div className="p-4 space-y-4">
                        ...
                        <select
                            id="sort-options"
                            value={sortOption}
                            onChange={(e) => setSortOption(e.target.value as 'name' | 'price-asc' | 'price-desc')}
                        ...
                        {/* Items List */}
                        {popupItems.length === 0 ? (
                        <p className="text-gray-600">Không có linh kiện tương thích.</p>
                        ) : (
                            ...
                            {popupItems
                                ...
                                >
                                    ...
                                    <div className="text-gray-600">
                                        {item.price 
                                            ? <div className="text-base font-semibold leading-none text-primary">{formatPrice(item.price) + " đ"}</div>
                                            : <div className="text-base font-semibold leading-none text-red-500">Liên hệ</div>
                                        }
                                    </div>
                                    </div>
                                </div>
            ...
            <div className="summary flex flex-col space-y-4 p-4 bg-white rounded shadow col-span-2 font-bold">
                <div className="total-price">
                    <p className="text-lg font-bold text-primary">Chi phí dự tính: {formatPrice(totalPrice.toString())} đ</p>
                </div>
                <button
                ...
                <div className="compatibility-state text-center py-2 px-4 bg-blue-100 rounded">
                    <p className="text-zinc-800 font-bold">Trạng thái tương thích: {isCompatible ? 'Bình thường' : 'Lỗi tương thích'}</p>
                </div>
                <div className="total-wattage text-center py-2 px-4 bg-blue-100 rounded">
                    <p className="text-zinc-800 font-bold">Công suất ước tính: {totalWattage} W</p>
                </div>
                <button
