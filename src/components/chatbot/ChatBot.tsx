...
export default function Chatbot() {
    ...
    useEffect(() => {
        if (isOpen && messages.length === 0) {
            setMessages([{
                type: 'text',
                data: { text: 'Xin chào! Tôi là trợ lý ảo, tôi có thể giúp bạn tìm kiếm thông tin hoặc đề xuất cấu hình PC. Hãy cho tôi biết bạn cần hỗ trợ gì nhé!' },
                sender: 'bot'
            }]);
        }
    }, [isOpen]);
    ...
    const sendMessage = async (message: string) => {
        ...
        try {
            ...
            setMessages((prev) => [
                ...
                {
                    type: 'text',
                    data: { text: 'Xin lỗi, đã xảy ra lỗi khi xử lý yêu cầu của bạn.' },
                    sender: 'bot',
                },
            ]);
        } finally {
    ...
    return (
        <>
            ...
            {isOpen && (
                <div
                    className={`fixed ${
                        isFullscreen ? "inset-0" : "bottom-5 right-5 w-80 md:w-96 h-[500px]"
