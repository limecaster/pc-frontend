...
import { getAddresses, addAddress, updateAddress, deleteAddress } from "@/api/account";
...
const AddressManager: React.FC = () => {
    ...
    useEffect(() => {
        if (currentAddress.city && addressData && Object.keys(addressData).length > 0) {
            const provinceKey = Object.keys(addressData).find(
                (key) => addressData[key].name_with_type === currentAddress.city
            );
            ...
            if (provinceKey) {
                ...
                if (!Object.values(addressData[provinceKey]["quan-huyen"]).some(
                    d => d.name_with_type === currentAddress.district
                )) {
                    // Reset district and ward
                    setCurrentAddress(prev => ({ ...prev, district: "", ward: "" }));
                }
            }
        }
    ...
    useEffect(() => {
        if (currentAddress.city && currentAddress.district && addressData && Object.keys(addressData).length > 0) {
            const provinceKey = Object.keys(addressData).find(
                (key) => addressData[key].name_with_type === currentAddress.city
            );
            ...
            if (provinceKey) {
                const districtKey = Object.keys(addressData[provinceKey]["quan-huyen"]).find(
                    (key) => addressData[provinceKey]["quan-huyen"][key].name_with_type === currentAddress.district
                );
                ...
                if (districtKey) {
                    ...
                    if (!Object.values(addressData[provinceKey]["quan-huyen"][districtKey]["xa-phuong"]).some(
                        w => w.name_with_type === currentAddress.ward
                    )) {
                        // Reset ward
    ...
    const validateForm = () => {
        ...
        const requiredFields = ['fullName', 'phoneNumber', 'street', 'ward', 'district', 'city'];
        ...
        if (currentAddress.phoneNumber && !/^[0-9]{10,11}$/.test(currentAddress.phoneNumber)) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (isEditing && currentAddress.id) {
                ...
                setAddresses(addresses.map(addr => addr.id === currentAddress.id ? updated : addr));
                            ...
                            <div className="flex justify-between">
                                ...
                                <div className="flex space-x-2">
                                    ...
                                    <button
                                        ...
                                        title={addresses.length === 1 ? "Không thể xóa địa chỉ duy nhất" : ""}
                                    >
