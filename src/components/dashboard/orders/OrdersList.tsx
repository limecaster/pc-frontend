...
const OrdersList: React.FC = () => {
    ...
    const handlePayment = async (orderId: number) => {
        try {
            ...
            if (response.success) {
                ...
                router.push(response.redirectUrl || `/checkout/payment/${orderId}`);
            } else {
    ...
    return (
        <div className="space-y-6">
            <h2 className="text-2xl font-semibold text-gray-800">Đơn hàng của bạn</h2>
                ...
                <table className="w-full text-sm text-left text-gray-500">
                    <thead className="text-xs text-gray-700 uppercase bg-gray-50">
                        <tr>
                            ...
                            <th scope="col" className="py-3 px-6 text-right">Thao tác</th>
                        </tr>
                    </thead>
                    <tbody>
                        {orders.map(order => (
