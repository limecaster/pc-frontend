      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm">
          ...
          <div className="text-gray-600 mb-4">
            <p className="mb-1"><span className="font-medium">Họ tên:</span> {user.name}</p>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm">
          ...
          {user.addresses.length > 0 ? (
            <div className="text-gray-600 mb-4">
              <p className="font-medium">{user.addresses[0].fullName}</p>
              <p className="text-sm mb-1">{user.addresses[0].address}</p>
          ...
          <Link 
            ...
            className="text-primary hover:underline text-sm font-medium flex items-center justify-end"
          >
            {user.addresses.length > 1 ? 'Xem tất cả địa chỉ' : 'Thêm địa chỉ'} 
            <ChevronRightIcon className="ml-1 w-4 h-4" />
          </Link>
        </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm">
          ...
          <div className="grid grid-cols-3 gap-2 mb-4">
            <div className="bg-yellow-50 p-2 rounded text-center">
              <p className="font-semibold text-xl text-yellow-700">{user.orders.pending}</p>
              <p className="text-xs text-gray-600">Chờ xác nhận</p>
            </div>
            <div className="bg-blue-50 p-2 rounded text-center">
              <p className="font-semibold text-xl text-blue-700">{user.orders.shipping}</p>
              <p className="text-xs text-gray-600">Đang giao</p>
            </div>
            <div className="bg-green-50 p-2 rounded text-center">
              <p className="font-semibold text-xl text-green-700">{user.orders.completed}</p>
          ...
          {recentOrders.length > 0 ? (
            <div className="divide-y divide-gray-200">
              {recentOrders.map((order) => {
                const { label, className, icon } = getStatusLabel(order.status);
                return (
                  <div key={order.id} className="p-4">
                    ...
                    <div className="flex items-center mb-2">
                      ...
                      <div className="ml-3 flex-1 min-w-0">
                        <Tooltip content={order.items[0].name}>
              ...
              {recentlyViewedProducts.map((product) => (
                <div key={product.id} className="p-4 flex items-center">
                  ...
                  <div className="ml-3 flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500">{product.viewedAt}</p>
                  </div>
                  <div className="ml-3">
                    <p className="text-sm font-semibold text-primary">
