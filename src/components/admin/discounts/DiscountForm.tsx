...
import { Modal, Button, TextInput, Select, Textarea, Spinner } from 'flowbite-react';
                  ...
                  <div className="space-y-1">
                    {formData.productIds.map(id => (
                      <div key={id} className="flex justify-between items-center bg-gray-50 p-2 rounded">
                        <span className="text-sm">{selectedProductNames[id] || id}</span>
                        <Button 
                          color="light" 
                          size="xs"
                          onClick={() => {
                            const newIds = formData.productIds?.filter(i => i !== id) || [];
                    ...
                    >
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setFormData((prev) => {
                            const currentCategories = prev.categoryNames || [];
                            return {
                              ...prev,
                              categoryNames: isChecked
          ...
          <div className="mt-4">
            ...
            <div className="flex flex-col space-y-3">
              ...
              formData.customerIds.length > 0 ? (
                <div className="border rounded-lg p-2 max-h-60 overflow-y-auto">
                  <div className="text-sm text-gray-500 mb-2">
                    Đã chọn {formData.customerIds.length} khách hàng:
                  </div>
                  <div className="space-y-1">
                    {formData.customerIds.map(id => (
                      <div key={id} className="flex justify-between items-center bg-gray-50 p-2 rounded">
                        <span className="text-sm">{selectedCustomerNames[id] || id}</span>
                        <Button 
                          color="light" 
                          size="xs"
                          onClick={() => {
                            const newIds = formData.customerIds?.filter(i => i !== id) || [];
                            setFormData(prev => ({...prev, customerIds: newIds}));
                          }}
                        >
                          <HiX />
                        </Button>
